...
export async function POST(req: NextRequest) {
  try {
    ...
    const { creatorUsername, txHash } = Schema.parse(await req.json())
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `${me.displayName || me.username} subscribed on-chain! TX: ${txHash.slice(0, 10)}…`,
      },
    })

    return ok({ confirmed: true, expiresAt })
